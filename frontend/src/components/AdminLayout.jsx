...
export default function AdminLayout() {
  ...
  return <div className="notranslate min-h-screen bg-ink text-white" translate="no"><aside className={`fixed inset-y-0 left-0 z-40 hidden border-r border-white/[.06] bg-night transition-all lg:block ${collapsed ? 'w-20' : 'w-64'}`}>{side}</aside>{open && <div className="fixed inset-0 z-50 bg-black/70 lg:hidden" onClick={() => setOpen(false)}><aside className="relative h-full w-72 border-r border-white/10 bg-night" onClick={(event) => event.stopPropagation()}>{side}</aside></div>}<div className={`transition-all ${collapsed ? 'lg:pl-20' : 'lg:pl-64'}`}><header className="sticky top-0 z-30 flex h-20 items-center justify-between border-b border-white/[.06] bg-ink/85 px-5 backdrop-blur-xl"><button className="p-2 lg:hidden" onClick={() => setOpen(true)} aria-label="Open sidebar"><Menu /></button><div className="ml-auto text-right"><p className="text-sm">{admin?.name}</p><p className="text-xs text-white/35">{admin?.role?.replace('_', ' ')}</p></div></header><main className="mx-auto max-w-[1500px] p-5 md:p-8"><Outlet /></main></div></div>;
}
